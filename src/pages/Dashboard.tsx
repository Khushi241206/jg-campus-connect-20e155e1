...
const card = "bg-card rounded-lg border border-border p-4 card-hover";
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">{user?.program} • {user?.semester}</p>
      </div>
            ...
            {notices.slice(0, 4).map((n) => (
              <div key={n.id} className="p-2.5 rounded-lg bg-muted/50 text-sm">
                <div className="flex items-start gap-2">
                  ...
                  {n.urgent && (
                    <span className="ml-auto text-xs px-2 py-0.5 rounded-full bg-destructive/10 text-destructive font-medium">Urgent</span>
                  )}
                </div>
              </div>
            ))}
