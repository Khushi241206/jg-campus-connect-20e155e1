  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        <div className="h-10 w-10 md:hidden rounded-full overflow-hidden border border-border shrink-0">
          <img src={avatarAnanya} alt={user?.name || ""} className="h-full w-full object-cover" />
        </div>
        <div>
          <h1 className="text-lg md:text-2xl font-bold text-foreground">Welcome, {user?.name}</h1>
          <p className="text-xs md:text-sm text-muted-foreground">{user?.program} • {user?.semester}</p>
        </div>
      </div>
        ...
        {stats.map((s, i) => (
          ...
          >
            <div className="flex items-center gap-2 md:gap-3">
              <div className={`p-1.5 md:p-2 rounded-lg bg-muted ${s.color}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        ...
        <div className={card}>
          <div className="flex items-center justify-between mb-3 md:mb-4">
            ...
            <button onClick={() => navigate("/timetable")} className="text-xs text-primary hover:underline">View All</button>
          </div>
            ...
            {todayClasses.slice(0, 4).map((c, i) => (
              <div key={i} className="flex items-center justify-between p-2.5 rounded-lg bg-muted/50 text-sm">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{c.time} • {c.faculty}</p>
                </div>
                <span className={`shrink-0 ml-2 text-xs px-2 py-1 rounded-full font-medium
                  ${c.type === "Lab" || c.type === "Practical" ? "bg-accent/20 text-accent-foreground" : "bg-success/10 text-success"}`}>
                  {c.type || "Theory"}
                </span>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        ...
        <div className={card}>
          <div className="flex items-center justify-between mb-3 md:mb-4">
            ...
            <button onClick={() => navigate("/notices")} className="text-xs text-primary hover:underline">View All</button>
          </div>
          <div className="space-y-2">
      ...
      {/* Attendance Trend */}
      <div className={card}>
        <h3 className="font-semibold text-foreground text-sm flex items-center gap-2 mb-3 md:mb-4">
